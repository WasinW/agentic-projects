...
> ## 🔴 UPDATE 2026-07-19 — GROSS not NET (validated กับ AIA tenant data)
...
-- ############################################################################
-- GROSS vs NET — ⚠ VALIDATED กับ AIA data (2026-07-19) = table เป็น GROSS:
--   system.billing.usage บันทึก Genie DBU "ทั้งหมด" ตั้งแต่ DBU แรก (รวมส่วนที่อยู่ใน free tier).
--   หลักฐาน AIA (Q5): มี user เพียบที่ Genie DBU/เดือน < 150 แต่ "มี row" (0.10, 0.71, 1.16 ...) →
--   ถ้าเป็น net คนพวกนี้ต้องไม่มี row → สมมติฐาน "net" ถูก falsified.
--   ⇒ query นี้ (SUM(dbu × price)) = GROSS LIST cost = ตัวเลข "ก่อนหัก free tier":
--      ใช้ทำ ATTRIBUTION ได้ แต่ OVER-states บิลจริง. บิล/Portal ให้ใช้ §1b (หัก free per-user floor).
--   หมายเหตุ: 150 = pricing page (verify per region) · pooled ต่อ IDENTIFIED user · SP ไม่มี free tier.
--
...
SELECT
    ...
    u.sku_name,
    SUM(u.usage_quantity)                             AS gross_dbus,      -- GROSS (ก่อนหัก free tier — บิลจริง §1b)
    SUM(u.usage_quantity * p.pricing.effective_list.default) AS gross_list_usd  -- gross list (over-states บิล)
...
```sql
-- ############################################################################
-- ## SECTION 1b — BILLED Genie DBU (หัก free-tier per-user floor)  ⭐ VALIDATED GROSS 2026-07-19
-- ############################################################################
-- Free 150 LLM DBU pooled ต่อ IDENTIFIED user/เดือน (ข้ามทุก surface) → รวม DBU ต่อ user ก่อน แล้วหัก 150 (floor 0).
-- SP/agent (run_as เป็น UUID ไม่ใช่ email) ไม่มี free tier → ไม่หัก.
WITH per_user AS (
    SELECT date_trunc('MONTH', usage_date) AS usage_month,
           identity_metadata.run_as        AS user_identity,
           SUM(usage_quantity)             AS gross_genie_dbus,
           -- heuristic: run_as มี '@' = identified user (ได้ free 150) · UUID = SP/agent (ไม่ได้ free)
           CASE WHEN identity_metadata.run_as LIKE '%@%' THEN 150 ELSE 0 END AS free_dbus
    FROM system.billing.usage
    WHERE billing_origin_product = 'GENIE'
    GROUP BY ALL
)
SELECT
    usage_month,
    ROUND(SUM(gross_genie_dbus), 2)                          AS gross_dbus,       -- §1 (over-states บิล)
    ROUND(SUM(LEAST(gross_genie_dbus, free_dbus)), 2)        AS free_credit_dbus, -- free tier ที่แจกจริง
    ROUND(SUM(GREATEST(gross_genie_dbus - free_dbus, 0)), 2) AS billed_dbus       -- ⭐ ตัวที่จะเทียบ Portal
FROM per_user
GROUP BY usage_month ORDER BY usage_month DESC;

-- billed cost (list) ≈ billed_dbus × Genie effective_list rate.
--   ⚠ free tier pooled ข้าม SKU → per-SKU exact ต้อง allocate DBU ที่เหลือ (>150) ตาม SKU;
--   approximation ที่ใช้ได้: หัก 150 จาก total DBU/user แล้วคูณ "blended Genie rate" (= gross_list_usd/gross_dbus จาก §1).
-- แล้วคูณ effective_rate (§3, Portal÷list) → actual $ ที่ใกล้ Portal.
-- ⚠ ตัวเลข AIA (capture 2026-07): Genie SKU = PREMIUM_SERVERLESS_REAL_TIME_INFERENCE_* (usage_type=TOKEN),
--   SHARE SKU กับ MODEL_SERVING + VECTOR_SEARCH (COMPUTE_TIME) → Portal แยก Genie ไม่ได้ → reconcile ที่ meter (§2/§3).
```
...
| # | Watch-out | ผล |
|---|---|---|
| 1 | **table เป็น GROSS (validated AIA 2026-07-19)** — `SUM(dbu×price)` = gross list, OVER-states บิล → บิลจริงหัก free **per-user floor** `GREATEST(0,user_dbu−150)` (§1b); SP ไม่มี free tier | gross = attribution · billed = เทียบ Portal |
