# Order Book Analysis — Level 2 Data, Imbalance Detection & Footprint Charts

> **Axis 1 — Trading Strategies | Module 03 — Order Flow & Liquidity**
> Document: 01_order_book_analysis.md
...
2. [Level 2 Data (Market Depth) Interpretation](#2-level-2-data-market-depth-interpretation)
3. [Order Book Imbalance Detection](#3-order-book-imbalance-detection)
4. [Spoofing and Layering Detection](#4-spoofing-and-layering-detection)
5. [Absorption Patterns](#5-absorption-patterns)
6. [Iceberg Order Detection](#6-iceberg-order-detection)
7. [Mathematical Models for Order Book Imbalance](#7-mathematical-models-for-order-book-imbalance)
8. [Delta — Cumulative Buy vs Sell Volume](#8-delta--cumulative-buy-vs-sell-volume)
9. [Order Flow Footprint Charts](#9-order-flow-footprint-charts)
10. [Implementation Algorithm for Order Book Analysis](#10-implementation-algorithm-for-order-book-analysis)
...
The order book — also called the Limit Order Book (LOB) or Depth of Market (DOM) — is the real-time registry of all outstanding buy and sell limit orders at various price levels. It is the most granular representation of supply and demand available to traders.

### 1.1 Why Analyze the Order Book?

The order book reveals:
- **Where liquidity exists** — which price levels have substantial resting orders
- **Where liquidity is thin** — price levels where rapid moves may occur
- **Directional pressure** — imbalance between bid and ask depth
- **Institutional footprints** — large hidden orders (icebergs), absorption patterns
- **Manipulation signals** — spoofing, layering, quote stuffing

### 1.2 Limitations of Order Book Data

| Limitation | Description | Mitigation |
|-----------|-------------|-----------|
| **Non-binding** | Limit orders can be cancelled at any time | Track cancellation rates; weight by fill probability |
| **Spoofing** | Fake orders placed to mislead | Detect and filter spoof patterns |
| **Fragmentation** | Multiple venues have different books | Aggregate across exchanges |
| **Latency** | Data may be stale by the time processed | Minimize processing latency; use co-location |
| **Hidden liquidity** | Iceberg/hidden orders not fully visible | Detect via fill patterns |
| **Forex OTC** | No central order book for spot FX | Use CME futures as proxy |

### 1.3 Data Sources

| Market | Source | Update Rate | Depth Levels |
|--------|--------|-------------|-------------|
...
## 2. Level 2 Data (Market Depth) Interpretation
